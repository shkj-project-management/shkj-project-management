...
export default function PDFExport() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const pdfResult = await appClient.export.toPDF(reportData);
      setResult(pdfResult);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="PDF Export" subtitle="Export reports to PDF format" icon={FileText} />
      <GlassCard className="p-6">
        ...
        {result && (
          <div className="mt-6 p-4 rounded-lg bg-green-500/10 border border-green-500/30">
            <p className="text-sm text-green-400">PDF exported successfully (Job ID: {result.job_id})</p>
          </div>
        )}
      </GlassCard>
    </div>
