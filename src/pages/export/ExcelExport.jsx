...
export default function ExcelExport() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const excelResult = await appClient.export.toExcel(reportData);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Excel Export" subtitle="Export reports to Excel/CSV format" icon={Table} />
      <GlassCard className="p-6">
        ...
        {result && (
          <div className="mt-6 p-4 rounded-lg bg-green-500/10 border border-green-500/30">
            <p className="text-sm text-green-400">Excel exported successfully (Job ID: {result.job_id})</p>
          </div>
        )}
      </GlassCard>
    </div>
